...
if authentication_status is False:
    ...
    st.markdown("##")
    def mcs_btc_2(project_id,sigma_eth,mean,period_in_days):
      def get_data(data):
          date = []
          price = []
      ...
      price_levels = [
            ...
            float(round((price0 + 0.4 * price0),4)),   # current_price + 40%
            float(round((price0 + 0.6 * price0),4)),  # current_price + 60%
            float(round((price0 + 0.8 * price0),4)),  # current_price + 80%
            float(round((price0 + 2 * price0),4)),   # current_price + 200%
            float(round((price0 + 3 * price0),4)),   # current_price + 300%
            float(round((price0 + 4 * price0),4)),   # current_price + 400%
            float(round((price0 + 6 * price0),4)),   # current_price + 600%
            float(round((price0 + 10 * price0),4)),   # current_price + 1000%
            float(round((price0 + 15 * price0),4))   # current_price + 1500%
        ]
